    ...
    const handleApprove = async (id) => {
      try {
        ...
        if (response.ok) {
          ...
          // Update the local state immediately by modifying the testimonial value to approved (e.g., 1)
          setTestimonials(
            testimonials.map((testimonial) =>
              testimonial.id === id ? { ...testimonial, value: 1 } : testimonial
            )
          );
        } else {
          setMessage('Failed to approve the testimonial. Please try again.');
        }
      } catch (error) {
        setMessage('Error occurred while approving. Please try again.');
      }
    };
            ...
            testimonials.map((testimonial) => (
              <TestimonialCard key={testimonial.id}>
                <TestimonialDetails>
                  ...
                  <p>
                    {testimonial.value === 0 ? (
                      <ApproveButton onClick={() => handleApprove(testimonial.id)}>Approve</ApproveButton>
                    ) : (
                      <p style={{ color: "lightcoral", fontFamily: "cursive", fontSize: '15px' }}>Approved</p>
                    )}
                  </p>
                </TestimonialDetails>
